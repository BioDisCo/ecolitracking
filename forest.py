...
def enter_cell(cell_id, time, time_offsets, in_frame):
    ...
    # Second nested clade
    inner_clade = SubElement(outer_clade, 'clade', branch_length=f"{time}")
...
def exit_cell(cell_id, current_time, in_frame):
    ...
    for clade_element in root.findall('.//clade'):
        ...
        if name_element.text == str(cell_id):
            ...
            branch_length = current_time - enter_time
            clade_element.set('branch_length', str(branch_length))
    ...
    for clade_element in root.findall('.//clade'):
        ...
        if not name_element.text is None:    
            ...
            if cell_id in in_frame:
                # Enter length 
                enter_time = float(clade_element.attrib.get('branch_length'))
                branch_length = stopping_time - enter_time
                clade_element.set('branch_length', str(branch_length))
...
def prune_forest():
    ...
    # XPath expression to select clade elements with branch_length less than 1/3
    xpath_expr = ".//clade"
    ...
    for clade in clades:
        ...
        if branch_length and float(branch_length) < (1/3):
...
def save_forest(time_offsets):
    ...
    Phylo.draw(trees, time_offsets=time_offsets, do_show=False)
