...
def draw(
    ...
    **kwargs,
    ...
    for idx, tree in enumerate(trees):
        ...
        max_xmax = max(max_xmax, max(x_posns.values()))
        ...
        def draw_clade_lines(
            ...
            if not use_linecollection and orientation == "horizontal":
                axes.hlines(y_here + y_offset, x_start, x_here, color=color, lw=lw)
            elif use_linecollection and orientation == "horizontal":
                horizontal_linecollections.append(
                    mpcollections.LineCollection(
                        [[(x_start, y_here + y_offset), (x_here, y_here + y_offset)]], color=color, lw=lw
                    )
                )
        ...
        def draw_clade(clade, x_start, color, lw):
            ...
            draw_clade_lines(
                ...
                orientation="horizontal",
                y_here=y_here,
                x_start=x_start + time_offsets[idx],
                x_here=x_here + time_offsets[idx],
                color=color,
                lw=lw,
            )
        ...
        y_offset = y_offset + max(y_posns.values())
    ...
    axes.set_xlim(-0.05, config['forest_duration'] + 0.05)
    # Also invert the y-axis (origin at the top)
    # Add a small vertical margin, but avoid including 0 and N+1 on the y axis
    axes.set_ylim(0.8 + y_offset, 0.2)
    axes.get_yaxis().set_visible(False)
    
    for text in fig.findobj(match=matplotlib.text.Text):
        # Check if the text belongs to title or labels
        if text in axes.get_xticklabels() or text in axes.get_yticklabels():
            # Skip axis tick labels
            continue
        elif text.get_text() == axes.get_title():
            # Skip the plot title
            continue
        elif text.get_text() == axes.get_xlabel():
            # Skip the X-axis label
            continue
        # Modify properties for all text elements
        text.set_fontsize(6)
        # Modify x and y positions
        if len(text.get_text()) > 2:
            x_pos, y_pos = text.get_position()
            text.set_position((x_pos - 0.2, y_pos))  # Adjust the values as needed
